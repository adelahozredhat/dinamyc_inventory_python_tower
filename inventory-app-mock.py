...
class HaiinvInventory(object):
    ...
    def add_group(self,data, group, pre_name, fathername):
        haiinv_environments_pro_inv = ['tst', 'pre', 'pro', 'cnt']
        if self.tower_env == 'pre':
            for record in group:
                if record != 'default':
                    if record != 'hosts' and re.match(r"^(custom_)[\w]+", record) is None and record not in haiinv_environments_pro_inv:
                        name = record
                        if pre_name != '':
...
def main(args):
    ...
    if os.environ.get('HAIINV_INI', ''):
        config_files = [os.environ['HAIINV_INI']]
    else:
        config_files = [
            os.path.abspath(sys.argv[0]).rstrip('.py') + '.ini', 'haiinv-app-mock.ini'
        ]
